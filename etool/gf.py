...
class Round():
	...
	def get_period(self):
		# определяемся с временными границами в зависимости от запрашиваемого периода
		date = pendulum.now(tz='UTC')
		start_period=date
		end_period=date

		if self.period == 's_week':
			# берем ближайщий понедельник и откатываемся на 7 дней = понедельник прошедшей недели
			...
			self.message = "Диаграмма деления платежей по категориям за прошедшую неделю с {} по {}".format(start_period.format('DD.MM'), end_period.format('DD.MM'))
				...
				self.message = "Диаграмма деления платежей по категориям за {}".format(prev_month.format('MMMM'))
		    ...
		    if self.query:
			    ...
			    end_period = date
			    self.message = "Диаграмма деления платежей по категориям начиная с {}.".format(start_period.format("DD.MM.YYYY"))
		    else:
		        self.status = False
		...
		if self.query:
		    self.title = "Круговая диаграмма платежей по категориям\n за период c {} по {}".format(start_period.format('DD.MM'),end_period.format('DD.MM'))
	...
	def get_data_values(self):
		...
		if self.status:
			...
			query=self.query.filter(datetime__range=period).order_by('datetime')
	...
	def run(self):
		...
		if data:
			...
			ax.pie(data[1],labels=data[0],colors=colors,
				autopct='%1.2f%%', radius=1,
				pctdistance=0.8,
				wedgeprops=dict(width=0.6, edgecolor='w'))
			ax.set_title('{}'.format(self.title), color='#999999')

			filename = '/home/dcoxtm/project/media/grafics/round-{}-{}'.format(self.period,uuid.uuid4())
			fig.savefig(filename)

			return {'filename':filename, 'message':self.message}

		else:
			self.message = "Ваш первый платеж произошел, позднее конца запрашиваемого периода."
...
class Wall():
	...
	def get_period(self):
		...
		if self.period == 's_week':
			...
			self.message = "Диаграмма величины платежей за прошедшую неделю с {} по {}".format(start_period.format('DD.MM'), end_period.format('DD.MM'))

		elif self.period == 's_month':
				prev_month = date.subtract(months=1)
				start_period = prev_month.start_of('month')
				end_period = prev_month.end_of('month')
				self.message = "Диаграмма величины платежей за {}".format(prev_month.format('MMMM'))
		    ...
		    if self.query:
		        ...
		        self.message = "Диаграмма величины платежей начиная с {}\n !Внимание данные не позднее 2-х месячной давности.".format(start_period.format("DD.MM.YYYY"))
		    else:
		        self.status = False
	...
	def get_data_values(self):
		...
		if self.status:
			...
			if self.period == 's_full':
				x_data = [i.format('DD.MM') for i in (period[1].add(days=1)-period[0])]

	    # так как данные для x отличаются заранее определим 'контейнеры' для фасовки
			if self.period == 's_week':
			...
			# для двух оставшихся фасуем с в упорядоченный словарь сопостовляя данные
			# c оси x c датой сохранения платежа
			else:
				...
				# для месяца избавляемся от излишней информативности(не отображаем месяц в диаграмме)
				if self.period == 's_month':
	...
	def run(self):
		...
		if data:
			...
			filename = '/home/dcoxtm/project/media/grafics/wall-{}-{}'.format(self.period,uuid.uuid4())
			fig.savefig(filename)
