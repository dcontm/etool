    ...
    def post(self, request, format=None):
        ...
        if data["type"] == "confirmation":
            ...
            if user_message == "Начать":
                ...
                router(
                    payload=data["object"]["payload"], user_id=user_id, vk=vk
                )

            elif user_message.isdigit():
                if user:
                    if user.status:
                        paymount = Paymount.objects.create(
                            user=user,
                        ...
                        vk.messages.send(
                            ...
                            message="Категория платежа {},сумма {} руб.".format(
                                paymount.get_category_display(), user_message
                            ),
                        )
                        user.status = None
                        user.save()
                        ...
                        vk.messages.send(
                            ...
                            random_id=get_random_id(),
                            keyboard=keyboards.general_menu(),
                            message=("Вы не выбрали " "назначение платежа"),
                        )
                else:
                    vk.messages.send(
                        peer_id=str(user_id),
